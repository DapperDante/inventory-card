DROP PROCEDURE IF EXISTS find_sale_for_return_lifo;

DELIMITER //
CREATE PROCEDURE find_sale_for_return_lifo(
...
BEGIN
  ...
  IF total_available_stock >= in_quantity THEN
    WITH RECURSIVE ordered_stock AS (
      SELECT id, inventory_card_id, available_stock, date
        FROM sale_view
        WHERE inventory_card_id = in_inventory_card_id
          AND available_stock > 0
    ),
    stock_cte AS (
      ...
      SELECT
        ...
        inventory_card_id,
        available_stock,
        LEAST(available_stock, in_quantity) AS to_return,
        in_quantity - LEAST(available_stock, in_quantity) AS remaining
      FROM ordered_stock
      -- WHERE id = (SELECT MAX(id) FROM ordered_stock)
      WHERE id = (
        SELECT MIN(id) FROM ordered_stock
        WHERE date = (SELECT MAX(date) FROM ordered_stock)
      )
      ...
      SELECT
        ...
        os.available_stock,
        LEAST(os.available_stock, sc.remaining) AS to_return,
      ...
      JOIN stock_cte sc ON os.id = (
        ...
        SELECT MIN(id) FROM ordered_stock
        WHERE id < sc.id
          AND date = (SELECT MAX(date) FROM ordered_stock WHERE id < sc.id)
      )
